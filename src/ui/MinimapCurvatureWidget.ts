...
export class MinimapCurvatureWidget {
  ...
  draw(pathPoints: MinimapPathPoint[], samples: CurvaturePreviewSample[], speed: number): void {
    ...
    const maxForward = Math.max(
      1,
      ...pathPoints.map((point) => point.forward)
    );
    ...
    this.ctx.strokeStyle = 'rgba(186, 222, 255, 0.95)';
    this.ctx.beginPath();
    ...
    for (const sample of samples) {
      const severity = clamp((speed - sample.safeSpeed) / 16, 0, 1);
