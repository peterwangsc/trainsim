...
export class GrassLayer {
  ...
  private spawnSideClump(
    ...
    const slopeMask =
      1 -
      MathUtils.smoothstep(slope, this.config.slopeMin, this.config.slopeMax);
    if (slopeMask <= 0.05) {
      return bladeCount;
    }
    ...
    const densityThreshold =
      this.config.densityThresholdMin + rng.next() * 0.18;
    ...
    const bladeTarget = Math.max(
      1,
      Math.round(
        this.config.bladesPerClump * MathUtils.lerp(0.55, 1.35, density),
      ),
    );
    ...
    for (
      ...
      if (
        this.sampleTrackDistance(x, z) <
        this.config.trackClearDistance * 0.85
      ) {
        continue;
      }
      ...
      const localSlopeMask =
        ...
        MathUtils.smoothstep(
          localSlope,
          this.config.slopeMin,
          this.config.slopeMax,
        );
  ...
  private sampleDensity(
    ...
    return MathUtils.clamp(
      (macroDensity * 0.72 + patchDensity * 0.28) * slopeMask * trackBlend,
      0,
      1,
    );
  }
  ...
  private hash2D(x: number, z: number): number {
    const value =
      Math.sin(x * 127.1 + z * 311.7 + this.seed * 0.173) * 43758.5453123;
  ...
  private createMaterial(): MeshStandardMaterial {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader
        .replace("#include <common>", grassVertexCommon())
        .replace("#include <begin_vertex>", grassVertexMain());

      shader.fragmentShader = shader.fragmentShader
        .replace("#include <common>", grassFragmentCommon())
        .replace(
          "#include <color_fragment>",
          grassFragmentMain(this.config.tintVariation.toFixed(3)),
        );
    };
